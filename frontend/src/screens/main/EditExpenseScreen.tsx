...
export const EditExpenseScreen = () => {
  ...
  const [title, setTitle] = useState(getCleanTitle());
  const [amount, setAmount] = useState(expense.amount ? expense.amount.toString() : "");
  const [date, setDate] = useState<Date>(expense.expenseDate ? new Date(expense.expenseDate) : new Date());
    ...
    if (userId) {
      const fetchPets = async () => {
        try {
          ...
          setPets(petList);
          const currentPet = petList.find(p => p.id === expense.petId) || petList[0] || null;
          setSelectedPet(currentPet);
        } catch (error) {
          console.error("Failed to fetch pets for editing expense:", error);
          Alert.alert("오류", "반려동물 목록을 불러오지 못했습니다.");
        }
      };
      fetchPets();
    }
  }, [userId, expense.petId]);
  ...
  const handleUpdateExpense = async () => {
    ...
    try {
      ...
      Alert.alert("성공", "지출 내역이 성공적으로 수정되었습니다.", [
        {
          text: "확인",
          onPress: () => {
      ...
      Alert.alert("수정 실패", "지출을 수정하는 도중 서버 오류가 발생했습니다.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <View style={styles.container}>
      ...
      <View style={commonStyles.header}>
        <View style={commonStyles.headerLeft}>
          ...
          <Text style={commonStyles.headerTitle}>소비 지출 수정</Text>
        </View>
      </View>

      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        
        {/* 반려동물 선택 (수정 시에는 변경 불가하게 읽기전용 느낌으로 두거나 선택 가능하게 유지) */}
        <Text style={styles.label}>반려동물 *</Text>
        <TouchableOpacity
          style={[styles.selector, { opacity: 0.6 }]}
          activeOpacity={1}
          disabled={true}
        >
        ...
        >
          {loading ? (
            ...
            <Text style={styles.saveButtonText}>지출 기록 수정 완료</Text>
          )}
        </TouchableOpacity>
